...
Everyone builds their model with build_cnn() so the results table compares
tasks, not architectures. If M1 and M3 each write a different network, a
difference in scores could be caused by either one and there is no way to tell.
...
def default_callbacks(target, patience=5):
    ...
    Everyone using the same stopping rule means training length isn't a hidden
    variable when we compare results.
    """
    return [
