...
What belongs in this file
    Anything that is a property of the *dataset*: junk columns, missing images,
    grayscale files, normalisation, the train/val assignment.

What does NOT belong here
    Anything that is a *modelling decision you might want to undo and compare
    against* — merging rare articleType classes into subCategory, collapsing
    usage's micro-classes into "Other", class weighting, augmentation. Those
    live in the task notebooks, because the comparison is the experiment.
...
# Metadata
# --------------------------------------------------------------------------
def load_metadata(verbose: bool = True) -> pd.DataFrame:
    """Load styles_train.csv, clean it, and attach the fixed train/val split.

    Cleaning applied here (identical for every task):
      * drops the two junk trailing columns produced by stray commas
      * inner-joins against the images that actually exist on disk
      * adds a `_split` column holding the frozen train/val assignment
    ...
    if verbose:
        ...
        print(f"          {(df[_SPLIT_COL] == 'train').sum()} train / "
              f"{(df[_SPLIT_COL] == 'val').sum()} val")
    return df


def _attach_split(df: pd.DataFrame) -> pd.DataFrame:
    """Assign every row to train or val, once, for all tasks.

    Stratified on articleType. Seven classes have a single example, which
    sklearn cannot stratify — those rows are routed to train and are therefore
    unevaluable. That is a property of the data, not a bug; report it.
    """
...
# Images
# --------------------------------------------------------------------------
def _build_cache(ids: list[str], image_dir, cache_path) -> np.ndarray:
...
def load_images(ids, split_name: str = "train") -> np.ndarray:
...
def normalise(x: np.ndarray, stats: tuple | None = None):
    """Scale to [0,1] and standardise per channel.

    Channel statistics are computed on the training split only and passed back,
    so the same numbers can be reapplied to val and test. Computing them on all
    the data would leak validation information into training.
    """
...
# --------------------------------------------------------------------------
def get_split(target: str, normalised: bool = True, verbose: bool = True):
    """Return (X_train, X_val, y_train, y_val, label_encoder) for one target.

    The train/val row assignment is identical for every target — only rows whose
    label for *this* target is missing are removed. That is what makes results
    comparable across tasks and what lets the multi-task model share a split.
...
def get_images_only(normalised: bool = True):
    """All training images plus their metadata, no target filtering.

    For Task 4: rungs 1-3 of the retrieval ladder are unsupervised, and the
    labels are needed only afterwards to compute Precision@K.
    """
    df = load_metadata(verbose=False)
...
def load_test_images(normalised: bool = True):
    """The 5,829 unlabelled test images, in the order styles_prediction.csv lists
    them — so predictions line up row-for-row with the submission file."""
...
if __name__ == "__main__":
    ...
    for t in ["gender", "articleType", "season", "usage"]:
        print(f"  {t:12} {meta[t].isna().sum():3d} missing, {meta[t].nunique():3d} classes")
